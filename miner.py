...
def main(config: dict):
    # ========== MINER D: EARLY ADVANTAGE PARAMETERS ==========
    base_n_samples = 800  
    ...
    bt.logging.info(f"[MINER D] First iteration optimized: {n_samples_first_iteration} samples")
    ...
    with ProcessPoolExecutor(max_workers=1) as cpu_executor:
        while time.time() - start < 1800:
            ...
            if score_improvement_rate > 0.05:
                ...
                bt.logging.info(f"[MINER D] High improvement, BOOSTED samples: {n_samples}")
            elif score_improvement_rate > 0.02:
                n_samples = int(base_adaptive * 1.2)
                bt.logging.info(f"[MINER D] Good improvement, increased samples: {n_samples}")
            else:
            ...
            if iteration == 2 and not top_pool.empty and synthon_lib is None:
                try:
                    bt.logging.info("[MINER D] RAPID synthon library building...")
                    synthon_lib_start = time.time()
                    synthon_lib = SynthonLibrary(DB_PATH, rxn_id)
                    use_synthon_search = True
                    bt.logging.info(f"[MINER D] Synthon library ready in {time.time() - synthon_lib_start:.2f}s")
                except Exception as e:
            ...
            if iteration == 1:
                bt.logging.info(f"[MINER D] Iteration {iteration}: EFFICIENT initial sampling ({n_samples_first_iteration} samples)")
                ...
                bt.logging.info(f"[MINER D] Iteration {iteration}: EARLY synthon search (from iter 2)")
                ...
                if score_improvement_rate > 0.05:
                    ...
                    bt.logging.info(f"[MINER D] Low improvement ({score_improvement_rate:.4f}), using multi-range strategy")
                    ...
                    if has_very_high_score or is_late_stage:
                        ...
                        bt.logging.info(f"[MINER D] Generated {len(synthon_top1_df)} TOP-1 synthon candidates")
                        ...
                        bt.logging.info(f"[MINER D] Generated {len(synthon_tight_df)} TIGHT synthon candidates")
                        ...
                        bt.logging.info(f"[MINER D] Generated {len(synthon_medium_df)} MEDIUM synthon candidates")
                        ...
                        bt.logging.info(f"[MINER D] Generated {len(synthon_broad_df)} BROAD synthon candidates")
                    ...
                    bt.logging.info(f"[MINER D] Combined: {len(data)} total")
            ...
            bt.logging.info(f"[MINER D] Iteration {iteration}: {len(data)} Samples Generated in ~{gen_time:.2f}s")
            ...
            # ========== MINER D: CPU 3-STRATEGY EARLY START  ==========
            cpu_futures = []
            if not top_pool.empty and iteration > 1:  # >3 → >1 
                cpu_futures.append((
                    cpu_executor.submit(
                        _cpu_random_candidates_with_similarity,
                        iteration, 50, config,  # 40 → 50
                ...
                cpu_futures.append((
                    cpu_executor.submit(
                        _cpu_random_candidates_with_similarity,
                        iteration, 40, config,
                        top_pool.head(15)[["name", "smiles", "InChIKey"]],  # 20 → 15
                        seen_inchikeys, 0.70
                    ),
                    "medium-top15"
                ))
                ...
                for cpu_future, strategy_name in cpu_futures:
                    try:
                        ...
                        if not cpu_df.empty:
                            ...
                            bt.logging.info(f"[MINER D] CPU similarity ({strategy_name}) found {len(cpu_df)} candidates")
                    except TimeoutError:
                        pass
                    except Exception as e:
            ...
            bt.logging.info(
                f"[MINER D] Iter {iteration} || Time: {iter_total_time:.2f}s | Total: {total_time:.2f}s | "
